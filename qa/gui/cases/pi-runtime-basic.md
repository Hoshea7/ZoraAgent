...
- 发起一个包含思考或工具调用的长任务。
- 当前轮仍在运行时发送一条带图片附件的引导消息。

通过标准：

- 引导消息立即显示在当前会话中。
- 图片从输入区转移到引导消息，并随同一条消息交给当前 Runtime。
- Runtime 开始处理引导后，在该用户消息下方创建独立的 Assistant Turn，后续思考、工具和正文不得追加到上一条回复。
- 已经开始的思考或工具事件正常结束。
- 引导在思考期间到达时，思考结束后计划调用的工具不执行；不得等待该工具完成后才处理引导。
- 引导在工具已经开始执行后到达时，当前工具事件完成后处理引导。
- Runtime 接受引导后继续同一轮执行，最终回复体现引导内容。
- 同一条引导消息只被处理一次。
- 用户发送引导后立即停止时，尚未进入 Runtime transcript 的消息保留在 Zora 历史，并在下一轮重新进入上下文。
